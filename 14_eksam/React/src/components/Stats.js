...
export const Stats = ({ onAnalyse, onSelectLemma, onSelectWord }) => {
    ...
    const data = useMemo(() => [...lemmaArray.sort((a, b) => {
    ...
    const columns = useMemo(() => [
        {
            ...
            Cell: (props: {value: string}) => {
                ...
                return <span onClick={(e) =>  onSelectLemma
            (e.target.textContent)}>{word}</span>
            },
        },
        {
            Header: 'Sõnavormid',
            ...
            Cell: (props: { value: Array }) => {
                ...
                for(let i=0; i<sortedItems.length; i++){
                    ...
                    let content = (
                        <WordCell key={word} onSelectWord
                ={ onSelectWord} sendWord={word} sendCount={count} />
                      )
        ...
        },
        {
            ...
            accessor: 'ratio',
            Cell: (row) => {
                let content = Math.round(row.row.original.count/words.length*100*100)/100
                return (
                  <span>
                    {content}%
                  </span>
